...
export default function Template69(data) {
  return `

<table cellpadding="0" cellspacing="0" style="font-family:Georgia, serif;font-size:12px;color:#222;table-layout:fixed;">
  <tr>
    ...
    <td style="border-right:1px solid #ddd;padding-right:10px;">
      <div style="font-size:15px;font-weight:bold;color:#64748B;">${data.name}</div>
      <div>${data.title}</div>
      ${data.company ? `<div>${data.company}</div>` : ''}
    </td>
    <td style="padding-left:10px;line-height:1.4;">
      ${data.email ? `<div>E: <a href="mailto:${data.email}" style="color:#64748B;text-decoration:none;">${data.email}</a></div>` : ''}
      ${data.phone ? `<div>P: ${data.phone}</div>` : ''}
      ${data.website ? `<div>W: <a href="${data.website}" style="color:#64748B;text-decoration:none;">${data.website}</a></div>` : ''}
